...
8. Write `docs/sdlc/tasks/task-{slug}.md` with exactly these seven sections:
   ...
   - **Technical Notes** — real file paths/contracts found in step 6. Per README's Global Constraints Concise grounding rule: inline the smallest verbatim excerpt that supports each point, or a `file:line` pointer plus a one-line paraphrase — never the surrounding prose. This file is re-read in full by every downstream agent that touches it — unnecessary length here multiplies across every one of those reads.
