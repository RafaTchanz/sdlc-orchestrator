...
- **Input**: exactly one finding — severity (`NIT` or `MINOR` only) and description, with `file:line` evidence from `sdlc-qa.md`/`sdlc-reviewer.md` or a scenario description from `sdlc-stress.md` — routed from `sdlc-qa.md`, `sdlc-reviewer.md`, or `sdlc-stress.md`.
...
1. Read the finding and go directly to its `file:line` (or, for a scenario-based Stress finding with no `file:line`, locate the specific code path the scenario describes) — do not re-read the whole story or re-derive context you don't need for a single-line/single-block fix.
