...
- **Input**: `qa.md`, `review.md`, `stress.md` for one story + the round number each report is expected to carry (from the caller).
...
1. Any of `qa.md`, `review.md`, or `stress.md` missing → automatic **NOT READY**, pending that audit — never treat a missing input as an implicit pass. Any present report whose own `Round` field doesn't match the round number the caller stated for it is stale (a carryover from a prior round, not this story's current one) → treated identically to missing, forcing **NOT READY**.
