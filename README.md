...
- **Loop cap & escalation**: QA, Review, and Stress each get up to 3 rounds per story, tracked independently (Review and Stress share one counter, since they run in parallel and route on the worse of their two signals — or, for a `simple`-Complexity story, Review's own signal alone, since Stress is skipped; see Complexity classification below). An unresolved `NIT`/`MINOR` at round 3 escalates to `MAJOR` (back to the Coder squad); an unresolved `MAJOR` at round 3 escalates to `CRITICAL`/`BLOCKED` and stops at a human-decision gate. Full routing logic: [`skills/sdlc/references/phases.md`](skills/sdlc/references/phases.md).
...
- **Stress skip for `simple` work**: `sdlc-stress` is never dispatched for a story/task/bug whose `Complexity` is `simple` — every round, not just the first. `simple`'s own definition below (no new integration, no concurrency/async coordination) already excludes the scenarios Stress checks for (load, resource exhaustion, races, partial-failure recovery), so running it anyway would only ever confirm what the classification already implies. `sdlc-reviewer` and `sdlc-qa` still run in full regardless of `Complexity` — this skip narrows only the resilience-under-load check, never correctness or test-quality auditing. `sdlc-verdict` treats a missing `stress.md` as N/A (not as a missing/failing input) whenever the caller states `Complexity: simple`; any other missing report is still treated as `NOT READY`. Routing logic: [`skills/sdlc/references/phases.md`](skills/sdlc/references/phases.md) step 5d, [`skills/sdlc-task/references/loop.md`](skills/sdlc-task/references/loop.md) step 4, [`skills/sdlc-bug-fix/references/dispatch.md`](skills/sdlc-bug-fix/references/dispatch.md) step 4.
