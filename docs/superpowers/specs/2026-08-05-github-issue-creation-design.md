...
| Multi-repo epic routing                                        | Not a separate concern: this boundary rule guarantees every story file `sdlc-github-issue` reads already carries exactly one manifest `Repo` value by construction — the agent never chooses among repos | The split happens upstream, in `sdlc-scrum-master`, before `sdlc-github-issue` ever runs; there is no dispatch-routing decision left to make downstream.                                                                                                                                                                                                                                                                |
